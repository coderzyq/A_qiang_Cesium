...
class TerrainExcavation {
  ...
  createBottomSurface1(points) {
    if (points.length) {
      ...
      this.bottomSurface1 = new Cesium.Primitive({
        geometryInstances: new Cesium.GeometryInstance({
          geometry: Cesium.PolygonGeometry.createGeometry(polygon),
        }),
        appearance: appearance,
  ...
  createWellWall(bottomPos, positionList) {
    ...
    for (let i = 0; i < positionList.length; i++) {
      ...
      minHeights.push(this.ellipsoidToDegree(positionList[i]).altitude - 50);
      // console.log("minHeight", minHeight);
      // console.log("minHeight-1000", minHeight-1000);
    }
